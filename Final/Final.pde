...
void draw() {
  ...
  image(bande, width/3 - bande.width/2, height - bande.height - 10);

  drawIcone();
  //image(icone, 0, 4.0f/5.0f*height);
  image(icone, width/3 - bande.width/2 + 10, height - bande.height - 5);

  drawScore();
  //image(score, BOX_SIZE/7.0f + 20, 4.0f/5.0f*height);
  image(scoreBox, width/3 - bande.width/2 + icone.width + 20, height - bande.height - 5);

  drawBarChart();
  image(barChart, width/3 - bande.width/2 + icone.width + scoreBox.width + 30, height - bande.height - 5);

  hs.update();
  hs.display();
  image(scroll, width/3 - bande.width/2 + icone.width + scoreBox.width + 31, height - bande.height + 74);

  drawCamera();
  image(camera, 2 * width/3, height - bande.height - 115);
...
void drawCamera() {
  camera.beginDraw();
  {
    camera.background(interfaceColor);
    camera.image(input, 1, 1, camera.width - 2, camera.height - 2);
  }
  camera.endDraw();
}