...
void draw() {
  ...
  background(BG_COLOR);
   ...
   drawBande();
    ...
    image(bande, width/2 - bande.width/2, height - bande.height);
...
public void drawIcone() {
  ...
  icone.ellipse(icone.width/2 + (ball.location.x * (icone.width*1.0 / plate.bWidth)),icone.height/2 + (ball.location.z * (icone.height*1.0 / plate.bWidth)),RADIUS/2,RADIUS/2);
  }
  icone.endDraw();
}
